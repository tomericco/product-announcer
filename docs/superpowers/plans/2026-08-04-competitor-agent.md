...
- **And where a sweep IS the thing under test, assert on which rows it touched, never on raw call counts.** The sweep reads every matching row in the shared database, and other test files insert sources with the schema default `status: "active"`. `toHaveBeenCalledTimes(n)` and `not.toHaveBeenCalled()` are therefore racy by construction; assert that the call arguments do or do not include the ids and URLs this test seeded. Those assertions still fail against the defect they were written to catch, and cannot be perturbed by another file's data.
