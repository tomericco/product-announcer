...
**Only `runSchedulerTick` leaves this route.** The route also carries a
`CRON_SECRET` bearer check, `retryFailedDeliveries()`, and
`sweepUnresolvedEvents()` — none of which are cadence machinery. Dropping the
retries silently strands failed publishes; dropping the sweep stops ingested
change events from ever resolving into atomic updates, which is core ingestion
that specs 3 and 5 build on. Result:
...
Add `tests/app/api/cron/scheduler/route.test.ts` covering the auth gate: 401
without a valid `Bearer ${CRON_SECRET}`, 200 with one, the two jobs stubbed. The
check was deletable without a single test failing, which is the real defect.
