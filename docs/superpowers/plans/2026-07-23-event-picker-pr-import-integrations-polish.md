...
`onImport` branches on `pickerType`: for commits, submit `Array.from(selectedCommits.values())` and call `importCommits`; for PRs, submit `Array.from(selectedPRs.values())` and call `importPullRequests`. Pass `enabledTypes={["commit", "pull_request"]}` to `EventMultiSelect`, and set `submitLabel` to reflect the active type ("Import N commits" / "Import N PRs").

**CRITICAL — selection snapshot (do not regress):** the dialog stores selections as snapshot **maps** — `Map<string, CommitSelection>` for commits and `Map<string, PullRequestSelection>` for PRs — NOT bare key sets, and NOT re-derived from the transient loaded list at submit. `commits`/`pullRequests` are fully replaced whenever the repo tab or date filter (or type) changes, so re-deriving a selection from the current list silently drops any selected item no longer in view. Pass `selected={new Set(map.keys())}` to `EventMultiSelect`; in `onSelectedChange(nextKeys)`, reconcile into a new map — preserve entries already in the prior map, and materialize newly-added keys from the currently-loaded list (a key can only be newly checked from a visible row). Submit reads the map's values directly. This mirrors the fix already applied to the commit path in Task 6; apply the identical shape to the PR path. Because `pickerType` change clears the selection, only one type's map is populated at a time — you may keep two separate maps or one keyed by the active type.
