...
Recorded from the final whole-branch review's triage.

### Done after merge

- **`importBrandStyle` re-gated.** It had lost its `isOnboardingComplete` check in
  the wizard rewrite. `guardOnboardingStep(2)` protects the page, but the action is
  a public endpoint that can be replayed — and it runs a live page fetch plus an LLM
  derivation before any redirect, so an ungated replay cost real money on a tenant
  who was already done.
- **Notion fetch failures no longer misdiagnosed.** A network error or 5xx used to
  collapse into an empty list, showing "no databases are shared with this
  integration yet" — telling the user to fix a sharing setting that was never the
  problem. Tracked separately now, with its own copy.
- **`saveNotionDatabase` revalidates `/onboarding/connect`** as well as
  `/integrations`, now that onboarding is a second call site.
- **Empty-URL handling on step 2** matches step 1: an explicit `?error=empty` with a
  `destructive` message rather than a silent bounce.
- **Step 2's muted failure copy is now documented** as deliberate, so the next reader
  doesn't "fix" a routine soft outcome into an alarming red banner.
- **`email-domain.test.ts` got a discriminating assertion** — `mail.gmail.com`, a
  subdomain of a *listed* provider. The previous `mail.acme.com` case passed even
  under a buggy suffix match.

### Dropped

- ~~Migration 0036 backfill for in-flight tenants.~~ Moot: there were no live
  half-onboarded tenants at deploy time, only test workspaces.
- ~~Collapse the `/` → `/onboarding` double redirect.~~ One extra indexed single-row
  query on a redirect-only route. Real, but the fix touches the authenticated entry
  path for a saving nobody perceives.

### Still open

1. **`saveWorkspaceName` always redirects to `/onboarding/brand`** — a user on step 3
...
2. **`PERSONAL_EMAIL_DOMAINS` needs an owner.** The list deliberately favors false
...
and the invite bypass were never exercised against a running app. Everything here was
verified statically plus by the automated suite.
