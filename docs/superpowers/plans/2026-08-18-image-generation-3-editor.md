...
- Cover "From library" copies the render as-is, with no new render — but the picker resolves the open question above as option (a): it filters to cover-shaped (1.91:1, within `ASPECT_TOLERANCE`) images only, so a body-sized (1200×900) source is excluded from the cover picker rather than being silently reused at the wrong aspect. The two OTHER entrances to the cover slot — a direct `setCoverFromImage` call, and `uploadImageFile` with `role: "cover"` — have no such shape check; that is an accepted gap for those paths (a user uploading their own cover, or a direct API caller, is expected to bring an appropriately-shaped image), not a defect, since the picker-level filter is specifically what protects the "silent reuse via picker" scenario this open question was about.
