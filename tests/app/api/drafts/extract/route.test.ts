...
describe("POST /api/drafts/extract", () => {
  ...
  // The stored body of a published release is what already went out to users;
  // splitting it would rewrite that text. Reachable only from a stale tab or a
  // crafted request, which is why the route re-checks rather than trusting the UI.
