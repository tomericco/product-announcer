 ...
 * control, closing stops nothing, and the board card's own inline checklist
 * keeps reporting the run afterwards — which is also what covers a generation
 * started in another tab.
 *
 * Where this is mounted matters, in both callers. It must sit ABOVE anything
 * that a re-render of the accepted brief can unmount — the board renders it
 * beside its columns rather than inside the brief card (accepting removes that
 * card), and the brief editor renders it outside the `canDecide` gate.
 */
  ...
  return (
    <Dialog open={contentPieceId !== null} onOpenChange={(next) => !next && onClose()}>
      ...
      <DialogContent className="sm:max-w-md" showCloseButton={false}>
        ...
        {contentPieceId !== null && (
          <GenerationRun key={contentPieceId} contentPieceId={contentPieceId} onClose={onClose} />
        )}
      </DialogContent>
    </Dialog>
  );
...
function GenerationRun({
  ...
  return (
    <>
      ...
      {/* The existing poll, unchanged and not duplicated. The refresh is
